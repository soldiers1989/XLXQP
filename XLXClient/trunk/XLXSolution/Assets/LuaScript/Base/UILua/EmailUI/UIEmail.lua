...
function ResetMailListItemContent(mailItem, mailInfo)
    ...
    if mailInfo.nSendType == MAIL_TYPE.INVITE or mailInfo.nSendType == MAIL_TYPE.PROMOTER  or mailInfo.nSendType == MAIL_TYPE.EXCHANGE then
...
function RefreshDetailPartContent()
    if detailPartMailInfo == nil then
        ...
        if detailPartMailInfo.nSendType == MAIL_TYPE.INVITE then
            if GameData.AgencyInfo.IsAgent == 0 then
                detailPart:Find('BottomButtons/ButtonApply').gameObject:SetActive(true)
            end
        elseif detailPartMailInfo.nSendType == MAIL_TYPE.PROMOTER or detailPartMailInfo.nSendType == MAIL_TYPE.PASSWORD or detailPartMailInfo.nSendType == MAIL_TYPE.EXCHANGE then
...
function HandleNotifyEmailRoleInfo(eventArg)
    ...
    if eventArg ~= nil then
        ...
        if eventArg.VipLevel > -1 and GameData.RoleInfo.VipLevel > 2 and eventArg.CanSendGold == 1 then
...
function SendPartSendButtonOnClick()
    if sendToInfo == nil then
        CS.BubblePrompt.Show(data.GetString("Send_Mail_Error_1"), "UIEmail")
        return
    end
