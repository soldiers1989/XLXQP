...
local m_BetLimitTime = 1.3			-- 下注倒计时限制时间
...
function InitRoomBaseInfoOfLimit(roomConfig)
    ...
    --roomLimit:Find('Item1/Value'):GetComponent("Text").text = lua_NumberToStyle1String(roomConfig.BettingLongHu[1]) .. "-" .. lua_NumberToStyle1String(roomConfig.BettingLongHu[2])
    --roomLimit:Find('Item2/Value'):GetComponent("Text").text = lua_NumberToStyle1String(roomConfig.BettingBaoZi[1])
    --roomLimit:Find('Item3/Value'):GetComponent("Text").text = lua_NumberToStyle1String(roomConfig.BettingJinHua[2]) .. "/" .. lua_NumberToStyle1String(roomConfig.BettingBaoZi[2])
    ...
    roomLimit:Find('Item2/Value'):GetComponent("Text").text = lua_NumberToStyle1String(GameData.RoomInfo.CurrentRoom.BetBaoZiMin)
    roomLimit:Find('Item3/Value'):GetComponent("Text").text = lua_NumberToStyle1String(GameData.RoomInfo.CurrentRoom.BetJinHuaMax) .. "/" .. lua_NumberToStyle1String(GameData.RoomInfo.CurrentRoom.BetBaoZiMax)
end
...
function InitRoomBaseInfoOfRoomInfo()
    ...
    -- else
    --     print('百人厅', GameData.RoomInfo.CurrentRoom.TemplateID)
...
function RefreshBankerInfo(arg)
    if arg == 1 then
        ...
        bankerRoot:Find('Name'):GetComponent("Text").text = bankerInfo.strLoginIP
        --bankerRoot:Find('BankerIcon'):GetComponent("Image"):ResetSpriteByName(GameData.GetRoleIconSpriteName(bankerInfo.HeadIcon))
        local _bankerIcon = bankerRoot:Find('BankerIcon'):GetComponent("Image")
        _bankerIcon:ResetSpriteByName(GameData.GetRoleIconSpriteName(bankerInfo.HeadIcon))
...
function InitRoomBaseInfoOfMineInfo()
    this.transform:Find('Canvas/RoleInfo/Name'):GetComponent("Text").text = GameData.RoleInfo.IPLocation
...
function ResetMineInfoOfGoldCount(arg, isInit)
    ...
    this.transform:Find('Canvas/RoleInfo/GoldCount'):GetComponent("Text").text = tostring(lua_GetPreciseDecimal(GameConfig.GetFormatColdNumber(displayCount),2))
...
function RefreshGameRoomToEnterGameState(roomState, isInit)
    ...
    RefreshFalseDealOfGameRoomByState(roomState, isInit)
...
function RefreshPokerCardPartOfGameRoomByState(roomState, isInit)
    ...
    if isInit then
        ...
        if roomState >= ROOM_STATE.DEAL then
            ...
            local settlementRoot = this.transform:Find('Canvas/PokerHandle/Result')
            settlementRoot.gameObject:SetActive(true)
            RefreshPokerCardSpriteOfTable()
        ...
        if roomState == ROOM_STATE.DEAL then
            ...
            this:DelayInvoke(0.3,function ()
                POKER_CARDS[1]:GetComponent("Image"):ResetSpriteByName(GameData.GetPokerCardSpriteName(GameData.RoomInfo.CurrentRoom.Pokers[1]))
                POKER_CARDS[4]:GetComponent("Image"):ResetSpriteByName(GameData.GetPokerCardSpriteName(GameData.RoomInfo.CurrentRoom.Pokers[4]))
                GameObjectSetActive(POKER_CARDS[1]:Find('back').gameObject,false)
                GameObjectSetActive(POKER_CARDS[4]:Find('back').gameObject,false)
                RefreshSettlementPartOfPokerCard(1)
                RefreshSettlementPartOfPokerCard(4)
                PlaySoundEffect(4)
            end)

            this:DelayInvoke(0.9,function()
                -- body
                POKER_CARDS[2]:GetComponent("Image"):ResetSpriteByName(GameData.GetPokerCardSpriteName(GameData.RoomInfo.CurrentRoom.Pokers[2]))
                POKER_CARDS[5]:GetComponent("Image"):ResetSpriteByName(GameData.GetPokerCardSpriteName(GameData.RoomInfo.CurrentRoom.Pokers[5]))
                GameObjectSetActive(POKER_CARDS[2]:Find('back').gameObject,false)
                GameObjectSetActive(POKER_CARDS[5]:Find('back').gameObject,false)
                RefreshSettlementPartOfPokerCard(2)
                RefreshSettlementPartOfPokerCard(5)
                PlaySoundEffect(4)
            end)
            this:DelayInvoke(1.5,function ()
                POKER_CARDS[3]:GetComponent("Image"):ResetSpriteByName(GameData.GetPokerCardSpriteName(GameData.RoomInfo.CurrentRoom.Pokers[3]))
                POKER_CARDS[6]:GetComponent("Image"):ResetSpriteByName(GameData.GetPokerCardSpriteName(GameData.RoomInfo.CurrentRoom.Pokers[6]))
                GameObjectSetActive(POKER_CARDS[3]:Find('back').gameObject,false)
                GameObjectSetActive(POKER_CARDS[6]:Find('back').gameObject,false)
                RefreshSettlementPartOfPokerCard(3)
                RefreshSettlementPartOfPokerCard(6)
                PlaySoundEffect(4)
            end)
        end
...
function RefreshFalseDealOfGameRoomByState(roomState, isInit)
    if roomState == ROOM_STATE.FALSEDEAL and isInit == false then
        ResetPokerCardToRestart()
        --RefreshPokerCardSpriteOfTable()
        HandleDealPokerCardToCardTable1(false)
        --RefreshCurrentGamePokerCardType()
    elseif roomState >= ROOM_STATE.FALSEDEAL and roomState < ROOM_STATE.DEAL and  isInit == true then
        ResetPokerCardToRestart()
        --RefreshPokerCardSpriteOfTable()
        HandleDealPokerCardToCardTable1(roomState > ROOM_STATE.FALSEDEAL)
        --RefreshCurrentGamePokerCardType()
        --ResetPokerCardTypeToVisibleByCardVisible()
    end
end
...
function RefreshPokerCardSpriteOfTable()
    for pokerIndex = 1, 6, 1 do
        ...
        GameObjectSetActive(POKER_CARDS[pokerIndex]:Find('back').gameObject,false)
    end
end

-- 扑克牌发送到牌桌上
function HandleDealPokerCardToCardTable1(isNoAnimation)
    local stateTotalLastTime = ROOM_TIME[ROOM_STATE.FALSEDEAL]
    local elapseTime = ROOM_TIME[ROOM_STATE.FALSEDEAL] - GameData.RoomInfo.CurrentRoom.CountDown
...
function HandleDealPokerCardToCardTable(isNoAnimation)
    ...
    m_PokerCardsRoot:GetComponent("AnimationControl"):Play(5, true)
end
...
function StartEnterCheckAnimation(roleType, isHandler, isAni)
    ...
    if not isHandler then
        handleCard2:ResetSprites(GameData.GetPokerCardBackSpriteNameOfBig(pokerCard2), GameData.GetPokerCardSpriteName(pokerCard2))
    else
    ...
    if not isHandler then
        handleCard3:ResetSprites(GameData.GetPokerCardBackSpriteNameOfBig(pokerCard3), GameData.GetPokerCardSpriteName(pokerCard3))
    else
...
function UpdateCheckCardCountDown()
    if isUpdateCheckCardCountDown then
        ...
        if GameData.RoomInfo.CurrentRoom.RoomState == ROOM_STATE.CHECK1 then
            local CloseCountDown = this.transform:Find('Canvas/BetRank/Area5Rank/RankFirst/CountDown').gameObject
            GameObjectSetActive(CloseCountDown,false)
        ...
        elseif GameData.RoomInfo.CurrentRoom.RoomState == ROOM_STATE.CHECK2 then
            local CloseCountDown = this.transform:Find('Canvas/BetRank/Area4Rank/RankFirst/CountDown').gameObject
            GameObjectSetActive(CloseCountDown,false)
...
function ResetPokerCardTypeToVisible(roleType, isActive)
    if roleType == 1 or roleType == 2 then
        ...
        this.transform:Find('Canvas/PokerHandle/Result/SettlementCards/PokerTypeImage' .. roleType).gameObject:SetActive(isActive)
        this.transform:Find('Canvas/DealPokers/PokerType' .. roleType).gameObject:SetActive(isActive)
        this.transform:Find('Canvas/DealPokers/PokerTypeImage' .. roleType).gameObject:SetActive(isActive)
    end
end
...
function SettlementAllAnimations(isInit)
    ...
    if elapseTime < 0.5 then
        this:DelayInvoke(0.5 - elapseTime, PlayLongPokerTypeAudio)
        -- 龙牌型
    end
    if elapseTime < 1.5 then
        this:DelayInvoke(1.5 - elapseTime, PlayHuBeginAudio)
        -- 虎
    end
    if elapseTime < 2.0 then
        this:DelayInvoke(2.0 - elapseTime, PlayHuPokerTypeAudio)
        -- 虎牌型
    end
    if elapseTime < 3.3 then
        this:DelayInvoke(3.3 - elapseTime, function() HandleShowGameResult(true) end)
        -- 显示印章
    else
...
function HandleShowGameResult(isAudio)
    ...
    -- 直接显示印章不播放音效
    HandleBetAreaAnimation(2)
    ResetPokerCardTypeToVisible(1, true)
    ResetPokerCardTypeToVisible(2, true)
    -- 押注区域开始闪烁
...
function HandleNotifyBetResult(eventArgs)
    if eventArgs.ResultType == 0 then
        ...
        if eventArgs.ResultType == 8 or eventArgs.ResultType == 13 or (eventArgs.ResultType >= 101 and eventArgs.ResultType <= 105) then
            CS.BubblePrompt.Show(string.format(data.GetString("Bet_Error_"..eventArgs.ResultType),lua_NumberToStyle1String(eventArgs.ErrorValue)),"LHDGameUI")
        else
            CS.BubblePrompt.Show(data.GetString("Bet_Error_" .. eventArgs.ResultType), "LHDGameUI")
        end
    end
end
...
local function FindDian(findString,Count)
    for Index=1,Count,1 do
        local Text = string.sub(findString,Index,Index)
        if Text == "." then
            return Index
        end
    end
...
function SetRankFirstInfo(firstTrans, firstInfo)
    ...
    --print("============================",string.len(firstInfo.strLoginIP))
    --[[local Length = string.len(firstInfo.strLoginIP)
    if Length > 7 then
        local Index = FindDian(firstInfo.strLoginIP,Length)
        local strLoginIP = string.sub(firstInfo.strLoginIP,1,Index-1)
        firstTrans:Find('Name'):GetComponent("Text").text = strLoginIP
    else--]]
        firstTrans:Find('Name'):GetComponent("Text").text = firstInfo.strLoginIP
    --end
    ...
    if firstInfo.ID == 0 or firstInfo.Icon == "" then
        ...
        local _FirstIcon = firstTrans:Find('FirstIcon'):GetComponent("Image")
        _FirstIcon:ResetSpriteByName(GameData.GetRoleIconSpriteName(firstInfo.Icon))
    end
end
    ...
    if rankData ~= nil and #rankData > 0 then
        ...
        for index = 1, 3, 1 do
            ...
            if rankInfo ~= nil then
                local colorValue = "95BDB0FF"
                
                rankItem.gameObject:SetActive(true)
                local Length = string.len(rankInfo.strLoginIP)
                if Length > 13 then
                    local Index = FindDian(rankInfo.strLoginIP,Length)
                    local strLoginIP = string.sub(rankInfo.strLoginIP,1,Index-1)
                    rankItem.transform:Find('Name'):GetComponent("Text").text = string.format("<color=#%s>%s</color>", colorValue, strLoginIP)
                else
                    rankItem.transform:Find('Name'):GetComponent("Text").text = string.format("<color=#%s>%s</color>", colorValue, rankInfo.strLoginIP)
                end
...
function RankButtonOnClick()
    ...
    initParam.WindowData = GAME_RANK_TYPE.LHD_MONEY
    CS.WindowManager.Instance:OpenWindow(initParam)
    if JudgmentTimeInterval(GAME_RANK_TYPE.LHD_MONEY) == true then
        NetMsgHandler.SendRequestRanks(GAME_RANK_TYPE.LHD_MONEY)
    end
end
...
function WindowOpened()
    ...
    CS.EventDispatcher.Instance:AddEventListener(EventDefine.SyncUpdateGold, RefreshMineInfoOfGoldCount)
...
function WindowClosed()
    ...
    CS.EventDispatcher.Instance:RemoveEventListener(EventDefine.SyncUpdateGold, RefreshMineInfoOfGoldCount)
