...
function PaoPaoUIInstance.HandleShowChat(eventArg)
	...
	local position,roomType,senderEnum,sendIndex,roleType = eventArg.position,eventArg.roomType,eventArg.senderEnum,eventArg.sendIndex,eventArg.roleType
	--print('=====roleType:' .. roleType .. " chatIndex: "..chatIndex)

	local finalWidth = 0
	if position == 1 then --庄家发言
		PaoPaoUIInstance.ownerChat.UI:SetActive(true)
		PaoPaoUIInstance.ownerChat.Text.text = data.GetString("Chat_PaoPao_".. sendIndex)
	...
	elseif position == 2 then --龙发言
		PaoPaoUIInstance.longChat.UI:SetActive(true)
		PaoPaoUIInstance.longChat.Text.text = data.GetString("Chat_PaoPao_".. sendIndex)
		...
		PaoPaoUIInstance.huChat.Text.text = data.GetString("Chat_PaoPao_".. sendIndex)
...
function PaoPaoUIInstance.OnSelectItem(index)
	...
	if GameData.RoomInfo.CurrentRoom.BankerInfo.ID == GameData.RoleInfo.AccountID then--自己是庄家
		--给服务器发消息
		NetMsgHandler.SendChatPaoPao(1,ROOM_TYPE.BRJH,2,index,1)
	else
		--游戏状态判断
		local roomState = GameData.RoomInfo.CurrentRoom.RoomState
		if roomState >= ROOM_STATE.DEAL then
			...
			if GameData.RoomInfo.CurrentRoom.CheckRole1.ID == GameData.RoleInfo.AccountID then
				--给服务器发消息
				NetMsgHandler.SendChatPaoPao(2,ROOM_TYPE.BRJH,2, index,2)
			elseif GameData.RoomInfo.CurrentRoom.CheckRole2.ID == GameData.RoleInfo.AccountID then
				--给服务器发消息
				NetMsgHandler.SendChatPaoPao(3, ROOM_TYPE.BRJH,2,index,3)
			else
