...
-- 底注下限  陌生人 (小接龙,大接龙) 入场设定 离场设定
local BetMinValue = 1
--local BetMaxValue = 40
local IsLock = 0        -- 是否允许陌生人加入
local GameMode = 1  --  游戏模式(1小接龙 2大接龙)
--local RoundTimes = 1    -- (闷1圈 闷3圈)
local EnterValue = 2000
local LeaveValue = 400
-- 房间等级
local RoomLevel = 1
-- 房间类型配置
local mRoomConfigType = 6
...
function GetOpenRoomMaxLevel()
    ...
    for key, config in pairs(data.HongbaoroomConfig) do
        if config.Type == mRoomConfigType and config.IsOpen == 1 then
            if maxLevel < config.RoomLevel then
                maxLevel = config.RoomLevel
            end
        end
    end
    return maxLevel
...
function RefreshBetLevel()
    ...
    -- 浮点数取整
    local config = GameConfig.HongbaoroomConfigDataByTypeLevel(mRoomConfigType,RoomLevel)
    print("创建配置数据:", config.LowLimit, mRoomConfigType, RoomLevel)
    BetMinValue = GameConfig.GetFormatColdNumber(config.Antes)
...
function CreateRoomButton_OnClick()
    ...
    if GameData.RoleInfo.GoldCount < EnterValue then
        ...
        NetMsgHandler.Send_CS_HB_Create_Room(RoomLevel,IsLock, GameMode, roomStyle, systemCost, ownerCost)
    end
