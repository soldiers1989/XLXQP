...
function RefreshLeftStatisticsTrendByStatistics(statistics)
    ...
    for roundIndex = 1, m_L_ItemCount, 1 do
        ...
        if roundItem == nil then
            print("LHD=111左侧数据 nil index:", roundIndex)
        else
...
function RefreshRightStatisticsTrendByStatistics(statistics)
    ...
    for index = 1, m_R_ItemCount, 1 do
        ...
        if trendItem ~= nil then
            ...
            print("LHD=222右侧数据 nil index:", index)
        end
    end
...
function RefreshBigEyeTrendByStatistics(statistics)
    ...
    for index = 1, m_R1_ItemCount, 1 do
        ...
        if trendItem ~= nil then
            ...
            print("LHD=333大眼仔 nil index:", index)
        end
    end
...
function RefreshSmallTrendByStatistics(statistics)
    ...
    for index = 1, m_R2_ItemCount, 1 do
        ...
        if trendItem ~= nil then
            ...
            print("LHD=333小路 nil index:", index)
        end
    end
...
function RefreshYueYouTrendByStatistics(statistics)
    ...
    for index = 1, m_R3_ItemCount, 1 do
        ...
        if trendItem ~= nil then
            ...
            print("LHD=333曱甴路 nil index:", index)
        end
    end
...
function GetTrendResultSpriteOfTrendItem(roundResult)
    if CS.Utility.GetLogicAndValue(roundResult, LHD_WIN_CODE.LONG) == LHD_WIN_CODE.LONG then
        return 'sprite_Trend_Icon_1'
    elseif CS.Utility.GetLogicAndValue(roundResult, LHD_WIN_CODE.HU) == LHD_WIN_CODE.HU then
        return 'sprite_Trend_Icon_2'
    elseif CS.Utility.GetLogicAndValue(roundResult, LHD_WIN_CODE.HE) == LHD_WIN_CODE.HE then
...
function GetTrend2IconNameByValue(value)
    if CS.Utility.GetLogicAndValue(value, LHD_WIN_CODE.LONG) == LHD_WIN_CODE.LONG then
        return 'sprite_hong'
    elseif CS.Utility.GetLogicAndValue(value, LHD_WIN_CODE.HU) == LHD_WIN_CODE.HU then
        return 'sprite_lan'
    elseif CS.Utility.GetLogicAndValue(value, LHD_WIN_CODE.HE) == LHD_WIN_CODE.HE then
        return 'sprite_lv'
    else
        return 'sprite_lv'
    end
...
function GetTrend2UpdateValue(value)
    if CS.Utility.GetLogicAndValue(value, LHD_WIN_CODE.LONG) == LHD_WIN_CODE.LONG then
        return LHD_WIN_CODE.LONG
    elseif CS.Utility.GetLogicAndValue(value, LHD_WIN_CODE.HU) == LHD_WIN_CODE.HU then
        return LHD_WIN_CODE.HU
    else
        return LHD_WIN_CODE.HE
    end
end
...
function InitPrediction()
    ...
    this.transform:Find('button01'):GetComponent("Button").onClick:AddListener(function () OnPredictionOnClick(1) end )
    this.transform:Find('button02'):GetComponent("Button").onClick:AddListener(function () OnPredictionOnClick(2) end )
end
...
function RefreshPreZXBtnState()
    ...
    if mCanPreBigEye then
        ...
        local tResultData = { value = 1, x = tPreData.x, y = tPreData.y, pos = tPreData.pos}
