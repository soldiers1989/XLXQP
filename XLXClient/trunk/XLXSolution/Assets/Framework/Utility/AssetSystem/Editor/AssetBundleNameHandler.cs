...
public class AssetBundleNameHandler
{
    ...
    public static void RefreshAssetBundleName()
    {
        ...
        RefreshUIPrefabAssetBundleBundleName();
    }
                    ...
                    if (textureImporter.textureType == TextureImporterType.Sprite)
                    {
                        ...
                        else
                        {
                            ...
                            if (string.IsNullOrEmpty(textureImporter.assetBundleName))
                            {
                                ...
                                Debug.LogErrorFormat("Texture[{0}] not set AssetBundle Name to build assetbundles.", abName);
                            }
                            else
                            {
                                // 已经设置bundle name 则需要统一处理后缀
                                string abName = Path.GetFileNameWithoutExtension(textureImporter.assetBundleName);
                                textureImporter.assetBundleName = abName + EditorAppDefine.AssetBundleExtension;
                            }
            ...
            for (int index = 0; index < fileFullNameArray.Length; index++)
            {
                ...
                if (importer != null)
                {
                    ...
                    if (asset != null)
                    {
                        ...
                        {
                            // 已经设置bundle name 则需要统一处理后缀
                            string abName = Path.GetFileNameWithoutExtension(importer.assetBundleName);
                            importer.assetBundleName = abName + EditorAppDefine.AssetBundleExtension;
                        }
                    }
                }
            }
