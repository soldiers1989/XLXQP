...
namespace Common.AssetSystem
{
    ...
    public partial class AssetBundleManager : ManagerBase<AssetBundleManager>
    {
...
#endif
            AssetBundle assetBundle = LoadAssetBundle(AppDefine.AssetBundleManifestName, false);
            if (assetBundle != null)
            {
        ...
        {
#if UNITY_EDITOR
            if (!AppDefine.OpenHotfix) // 编辑器模式下，未开启热更时，加载初始路径资源
            {
                return string.Format("{0}/{1}/{2}", AppDefine.LOCAL_INIT_DATA_PATH, AppDefine.AssetBundlesPath, fileName);
            }
            else
#endif
            {
                return string.Format("{0}/{1}/{2}", AppDefine.LOCAL_DATA_PATH, AppDefine.AssetBundlesPath, fileName);
            }
        }
