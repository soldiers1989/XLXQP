...
public static class Utility
{
    ...
    /// <returns></returns>
    public static IEnumerator ConvertURLToIPAddress(string url, Action<string> callBack)
    {
        string ipAddress = string.Empty;
        bool isOk = false;
        try
        {
            Dns.BeginGetHostAddresses(url, (asyncCallBack) =>
            {
                try
                {
                    IPAddress[] ipAddressArray = Dns.EndGetHostAddresses(asyncCallBack);
    ...
    public static void CreateBarcode(RawImage Img, string textForEncoding, int width, int height)
    {
    ...
    {
#if UNITY_IOS && !UNITY_EDITOR
        IOSAlbumCamera.Instance.OnSaveImage2ios();
#elif UNITY_ANDROID && !UNITY_EDITOR
        IOSAlbumCamera.Instance.OnSaveImage2Android();
#else
        // Window or EDITOR
        string fileName = string.Format("QRCode-Screenshot-{0}-{1}.png", DateTime.Now.ToString("yyyyMMdd"), DateTime.Now.ToString("hhmmssfff"));
        string path = Application.persistentDataPath + "/QRCode";
        Debug.LogFormat("Path 1:{0}", path);
        //判断目录是否存在，不存在则会创建目录  
    ...
    /// 转换微信头像的级别
    /// </summary>
    /// <param name="wxIconUrl">微信头像URL地址</param>
    /// <param name="toLevel">微信头像目标级别</param>
    /// <param name="wxUrl">微信头像URL地址开头</param>
    /// <returns></returns>
    public static string ConvetWXHeadUrlLevel(string wxIconUrl, string toLevel = "96", string wxUrl = "http://wx.qlogo.cn/mmopen")
    {
        if (wxIconUrl.StartsWith(wxUrl))
        {
            int index = wxIconUrl.LastIndexOf("/");
            string value = wxIconUrl.Substring(index + 1);
            int level = 0;
            if (int.TryParse(value, out level))
            {
                wxIconUrl = wxIconUrl.Substring(0, index + 1) + toLevel;
            }
        }
        return wxIconUrl;
    }
    ...
    /// 清理本地头像缓存
    /// </summary>
    public static void ClearLocalHeadIconCache()
    {
        Common.FileUtility.DeleteDirectory(AppDefine.LOCAL_HEADICON_PATH, true);
    }

    /// <summary>
    /// [自动旋转]:横向方向，靠左
    /// </summary>
    public static void AutorotateToLandscapeLeft()
    {
    ...
    /// [自动旋转]:横向方向，靠右
    /// </summary>
    public static void AutorotateToLandscapeRight()
    {
        Screen.autorotateToLandscapeLeft = true;
        Screen.autorotateToLandscapeRight = false;
        Screen.autorotateToPortrait = false;
    ...
    /// [自动旋转]: 竖直 正方向
    /// </summary>
    public static void AutorotateToPortrait()
    {
    ...
    /// [自动旋转]: 竖直 反方向
    /// </summary>
    public static void AutorotateToPortraitUpsideDown()
    {
        Screen.autorotateToLandscapeLeft = false;
        Screen.autorotateToLandscapeRight = false;
        Screen.autorotateToPortrait = false;
