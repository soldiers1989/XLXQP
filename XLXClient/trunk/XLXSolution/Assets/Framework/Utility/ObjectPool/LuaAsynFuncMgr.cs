...
public class LuaAsynFuncMgr : MonoBehaviour
{
    ...
    public XLua.LuaTable ParseJsonToLuaTable(string paramjson)
    {
        // 解析json 转换为lua的table
        Dictionary<string, object> dict = MiniJSON.Json.Deserialize(paramjson) as Dictionary<string, object>;
        if (dict == null || dict.Count == 0)
        {
            Debug.LogErrorFormat("ParseJsonToLuaTable传的json出错:{0}", paramjson);
            return null;
        }
        var table = LuaManager.LuaEnv.NewTable();
        foreach (var item in dict)
        {
            // Debug.LogFormat(string.Format("key = {0}, value = {1}", item.Key, item.Value));
