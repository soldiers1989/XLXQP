...
class WithdrawController extends BaseController
{
    ...
    public function withdrawalApplication()
    {
        ...
        if (1 == I('get.isExecl/d')) {
            $this->setExeclData(C('cellName.withdraw_application'), $export['data'], '提现申请');
        }
        G('end');
        if (G('begin', 'end') > 2) {
    ...
    public function specialOrderProcess()
    {
        ...
        if (1 == I('get.isExecl/d')) {
            $this->setExeclData(C('cellName.special_order'), $layui_data['data'], '特殊订单处理');
        }
    ...
    public function withdrawalsRecord()
    {
        ...
        if (1 == I('get.isExecl/d')) {
            $this->setExeclData(C('cellName.withdraw_record'), $export['data'], '提现记录');
        }
    ...
    public function orderInquiry()
    {
        ...
        if (1 == I('get.isExecl/d')) {
            $this->setExeclData(C('cellName.order_inquiry'), $layui_data['data'], '订单查询');
        }
...
//            $channel = M("bk_apkchannel")->where( array("bk_ID" => $id ) )->getField('bk_ChannelID');
//
//            $str = pack("S", strlen($apk)) . $apk . pack("S", strlen($channel)) . $channel . pack("S", strlen($editWeights)) . $editWeights;
//
//            $socket_result = SendToGame( C('socket_ip'), 30000, 1004, $str );
